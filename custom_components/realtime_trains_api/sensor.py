...
async def async_setup_platform(
    ...
    coordinator = RealtimeTrainsUpdateCoordinator(
        ...
        auto_adjust_scans=bool(config.get(CONF_AUTOADJUSTSCANS, False)),
    )
    ...
    for idx, raw_query in enumerate(queries or []):
        ...
        query_key = build_query_key(origin, destination, platforms, time_offset)
...
class RealtimeTrainLiveTrainTimeSensor(CoordinatorEntity, SensorEntity):
    ...
    def extra_state_attributes(self):
        ...
        attrs[ATTR_DATA_STALE] = bool(getattr(self.coordinator, "data_stale", False))
        if getattr(self.coordinator, "last_successful_update", None):
            attrs[ATTR_LAST_SUCCESSFUL_UPDATE] = self.coordinator.last_successful_update.isoformat()
