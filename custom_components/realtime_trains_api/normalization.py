...
DEFAULT_TITLE = "Realtime Trains API"
LEGACY_TITLE_PATTERN = re.compile(r"^Realtime Trains API \([^()\n]{5}\.\.\.\)$")


def token_fingerprint(token: str) -> str:
    """Return a stable SHA-256 hex digest fingerprint for a token."""
    return hashlib.sha256(token.strip().encode("utf-8")).hexdigest()


def scrub_legacy_title(title: str | None) -> str:
    """Scrub legacy autogenerated token-bearing titles back to default title, preserving custom titles."""
    if not title:
        return DEFAULT_TITLE
    if LEGACY_TITLE_PATTERN.match(title.strip()):
        return DEFAULT_TITLE
    return title
